...
export default function Page() {
  ...
  const [efforts, setEfforts] = useState<{ id: string; label: string }[]>([])
  const [projectContext, setProjectContext] = useState<ProjectIntelligence | null>(null)
  ...
  const [links, setLinks] = useState(292)
  const [rechts, setRechts] = useState(370)
    ...
    fetch('/api/meta')
      ...
      .then((d) => {
        ...
        if (wieder) {
          ...
          setMode(wieder.mode ?? 'direct')
          setEffort(wieder.effort ?? 'high')
  ...
  // Claude-Konfiguration ist projektspezifisch. Beim Wechsel Rollen, Skills,
  ...
  useEffect(() => {
    ...
    try {
      const roh = localStorage.getItem('fleet.layout.v3')
      if (roh) {
        ...
        const klemm = (wert: unknown, standard: number) =>
          typeof wert === 'number' && Number.isFinite(wert)
            ...
            : standard
        setLinks(klemm(gespeichert.links, 292))
        setRechts(klemm(gespeichert.rechts, 370))
  ...
  useEffect(() => {
    localStorage.setItem('fleet.layout.v3', JSON.stringify({ links, rechts, ansicht }))
  ...
  useEffect(() => {
    ...
    es.addEventListener('tokens', (e) =>
      setTokens((alt) => ({ ...alt, ...JSON.parse((e as MessageEvent).data) }))
    )
  ...
  const effectiveEffort = mode === 'parallel' ? 'ultracode' : effort
  const cliPreview = `claude -p --output-format stream-json --input-format stream-json --verbose --model ${model} --effort ${effectiveEffort}${
    ...
    >
      {tab === 'konsole' && !schmal && (
        <div
          ...
          onDoubleClick={() => setLinks(links > 0 ? 0 : 292)}
      ...
      {tab === 'konsole' && !schmal && (
        <div
          ...
          onDoubleClick={() => setRechts(rechts > 0 ? 0 : 370)}
          title={t('layout.resizerTitle')}
        />
      )}
      ...
      {tab === 'konsole' && (schmal ? schublade !== 'links' : links === 0) && (
        <button
          ...
          style={{ left: 0 }}
          onClick={() => (schmal ? setSchublade('links') : setLinks(292))}
      ...
      {tab === 'konsole' && (schmal ? schublade !== 'rechts' : rechts === 0) && (
        <button
          className="aufklappen"
          style={{ right: 0 }}
          onClick={() => (schmal ? setSchublade('rechts') : setRechts(370))}
      ...
      {tab === 'konsole' && (!schmal || schublade === 'links') && (
        ...
        >
          <SessionSidebar
            ...
            efforts={efforts}
            effort={effort}
            onEffort={(value) => setEffort(value as EffortLevel)}
            mode={mode}
            onMode={setMode}
        ...
        <>
          ...
          >
            <div
              ...
              style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px 14px' }}
            >
              <div style={{ minWidth: 160, flex: '1 1 200px' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--color-neutral-500)' }}>
                  {session
                    ? `${session.project.replace(/^\/Users\/[^/]+/, '~')} · ${session.model}/${session.effort ?? 'high'} · ${session.mode ?? 'direct'}` +
              ...
              >
                <div className="seg">
                  ...
                  <label className="seg-opt" style={{ padding: '4px 10px', fontSize: 12 }}>
                    ...
                    {t('console.viewAnswer')}
                    {antwortenAnzeige.length > 1 ? ` (${antwortenAnzeige.length})` : ''}
                ...
                {session && (
                  <div className="hud-stat hud-mode">
                    <i className="ph ph-circles-three-plus" />
                    {session.mode ?? 'direct'}
                  </div>
                )}
              ...
              <div className="leer">
                <div style={{ maxWidth: 380 }}>
                  ...
                  <div style={{ fontSize: 12.5, lineHeight: 1.6 }}>
                    {t('console.emptyHint')}{' '}
                    <strong style={{ color: 'var(--color-neutral-300)' }}>
